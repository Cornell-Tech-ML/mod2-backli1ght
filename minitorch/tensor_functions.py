...
class Function:
    ...
    def apply(cls, *vals: Tensor) -> Tensor:
        """Apply the function to the given tensor values.

        "Args":
            vals: Tensor values to which the function will be applied.
...
class Add(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, Tensor]:
        """Compute the gradients for both input tensors in addition."""
...
class Mul(Function):
    ...
    @staticmethod
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, Tensor]:
        """Compute the gradients for both input tensors in multiplication."""
...
class Sigmoid(Function):
    @staticmethod
    def forward(ctx: Context, t1: Tensor) -> Tensor:
        """Compute the sigmoid activation of the input tensor."""
...
class ReLU(Function):
    @staticmethod
    def forward(ctx: Context, t1: Tensor) -> Tensor:
        """Compute the ReLU activation of the input tensor."""
        ctx.save_for_backward(t1)
...
class Exp(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        ...
        (t1,) = ctx.saved_values
        expVal = t1.f.exp_map(t1)
        return grad_output.f.mul_zip(expVal, grad_output)
...
class Sum(Function):
    ...
    def forward(ctx: Context, a: Tensor, dim: Tensor) -> Tensor:
        ...
        if dim is not None:
            ...
            return a.f.add_reduce(a, list(range(a.dims))).view(1)
...
class All(Function):
    @staticmethod
    def forward(ctx: Context, a: Tensor, dim: Tensor) -> Tensor:
        """Compute the logical AND of the input tensor along the specified dimension."""
        if dim is not None:
...
class LT(Function):
    ...
    @staticmethod
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, Tensor]:
        """Compute the gradient of the less than comparison."""
...
class EQ(Function):
    ...
    @staticmethod
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, Tensor]:
        """Compute the gradient of the equality comparison."""
        return grad_output.zeros(), grad_output.zeros()
...
class View(Function):
    ...
    def forward(ctx: Context, a: Tensor, shape: Tensor) -> Tensor:
        """View the input tensor with a new shape."""
    ...
    @staticmethod
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        """Compute the gradient of the view operation."""
...
class Copy(Function):
    ...
    @staticmethod
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        """Compute the gradient of the copy operation."""
        return grad_output
...
class MatMul(Function):
    ...
    @staticmethod
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, Tensor]:
        """Compute the gradients for both input tensors in matrix multiplication."""
...
def grad_central_difference(
    ...
    """Compute the gradient of a function using central difference.
    ...
    "Returns":
        float: The estimated gradient of the function at the specified argument.
    """
    x = vals[arg]
    up = zeros(x.shape)
    up[ind] = epsilon
...
def grad_check(f: Any, *vals: Tensor) -> None:
    """Check the gradients of a function using backpropagation and central difference.

    "Args":
        f: The function to check gradients for.
        vals: The input tensors to the function.

    "Raises":
        AssertionError: If the computed gradients do not match the expected values.
    """
    for x in vals:
